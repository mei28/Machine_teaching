...
textbook = 501
# 推定に使う教材数
test_textbook_list = [10, 100, 1000]
# 推定間に提示する教材数
between_textbook_list = [1, 10, 100]
# 組
k = 1
...
a = a[1:]
write_np2csv(
    a, '{}Omniscient/omni-{}.csv'.format(result_path, k))
...
a = a[1:]
write_np2csv(a, '{}Random/ran-{}.csv'.format(result_path, k))
print('{}: finished.'.format(k))
...
for t_num in test_textbook_list:
    for b_num in between_textbook_list:
        ...
        a = a[1:]
        write_np2csv(
            a, '{}Without_teacher/w_star/w-t{}b{}-{}.csv'.format(result_path, t_num, b_num, k))
